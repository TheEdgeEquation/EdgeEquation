...
def run_gotd(dry_run, no_graphic):
    ...
    try:
        ...
        # GOTD = best game total play (not NRFI/YRFI, not player props)
        game_plays = [p for p in all_plays
...
def run_potd(dry_run, no_graphic):
    ...
    try:
        ...
        # POTD = best player prop (K, SOG, 3PM, PTS etc)
        prop_plays = [p for p in all_plays
                      if p.get("prop_label") in ("K", "SOG", "3PM", "PTS", "AST", "REB")]
        top_prop = prop_plays[0] if prop_plays else None
 
        # First Inning Spotlight = best NRFI/YRFI
        nrfi_plays = [p for p in all_plays
...
def run_results(dry_run, no_graphic):
    ...
    try:
        ...
        try:
            ...
            mlb_game_projs = [p for p in todays_plays if p.get("sport") == "baseball_mlb"
            ...
            logger.info("Game hits: " + str(len(game_hits)) + " misses: " + str(len(game_misses)))
            logger.info("Pitcher hits: " + str(len(pitcher_hits)) + " misses: " + str(len(pitcher_misses)))
            if not dry_run:
