    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if code in (500, 502, 503, 504):
                wait = 2 ** attempt
                print(f"[POSTING] Transient {code} error. Retry {attempt}/{max_retries} in {wait}s")
                time.sleep(wait)
                continue

            print(f"[POSTING] Non‑retryable Tweepy error: {e}")
            raise
...
def post_tweet(text, image_path=None, account="ee", long_form=False):
    """
    Posts a single tweet. Raises on failure so retry logic can catch it.
    """
    client = _get_client()
    text = text[:MAX_CHARS]
...
def post_thread(tweets, account="ee"):
    ...
    try:
        ...
        logger.info(f"Thread posted: {len(ids)} tweets")
        return ids
