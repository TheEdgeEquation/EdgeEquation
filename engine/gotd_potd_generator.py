...
def _build_drivers(sport: str, game: dict) -> dict:
    ...
    # Handle both game_projector format (home_team/away_team)
    # and card data format (teams/away/home)
    away = (game.get("away_team") or
            game.get("away") or
            (game.get("teams", "") or "").split("@")[0].strip() or
            "Away")
    home = (game.get("home_team") or
            game.get("home") or
            (game.get("teams", "") or "").split("@")[-1].strip() or
            "Home")
 
    # Score/total fields vary by source
    proj_away = game.get("away_runs", game.get("away_score",
                game.get("away_goals", game.get("proj_score", ""))))
    proj_home = game.get("home_runs", game.get("home_score",
                game.get("home_goals", "")))
    total = game.get("total", game.get("proj_total", ""))
    edge = game.get("edge_percent", "")
    vegas = game.get("vegas_line", game.get("vegas_total", ""))
 
    # Build readable strings
    proj_score_str = f"{proj_away} - {proj_home}" if proj_away and proj_home else str(total)
    line_eff = f"Proj {total} vs Vegas {vegas}" if vegas else f"Proj total: {total}"
    edge_str = edge if edge else f"Total: {total}"
 
    # Pitching delta if pitchers available
    home_p = game.get("home_pitcher", "")
    away_p = game.get("away_pitcher", "")
    pitching_delta = f"{away_p} vs {home_p}" if home_p and away_p else "Favorable"
 
    return {
        "away": away,
        "home": home,
        "fighter_a": game.get("fighter_a", away),
        "fighter_b": game.get("fighter_b", home),
        "proj_total": str(total),
        "vegas_line": str(vegas) if vegas else "N/A",
        "edge_percent": edge_str,
        "variance": f"High — Proj {total}" if total else "Elevated",
...
def find_top_game(games: list) -> dict | None:
    """
    Find the top game for GOTD.
    Handles both edge_percent format and game_projector format.
    Uses edge_percent if available, otherwise picks highest total projection.
    """
    if not games:
        return None
    try:
        def sort_key(g):
            ep = g.get("edge_percent", "")
            if ep:
                return float(str(ep).replace("%", "").strip())
            total = g.get("total", g.get("proj_total", 0))
            return float(str(total))
        return max(games, key=sort_key)
    except Exception:
