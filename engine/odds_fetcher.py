...
def fetch_all_props():
    logger.info("Generating K props from model (primary)...")
    try:
        from engine.prop_generator import generate_k_props
        model_props = generate_k_props()
        if model_props:
            logger.info("Model generated " + str(len(model_props)) + " K props")
            return model_props
        else:
            logger.warning("Model generated no props")
    except Exception as e:
        logger.error("Model prop generation failed: " + str(e))
 
    logger.info("Trying Odds API fallback...")
