...
def get_pitcher_season_stats(player_id, season):
    try:
        ...
        avg_ip = round(ip / max(gs, 1), 2) if gs > 0 else 0.0
        avg_ip = min(avg_ip, MAX_IP_PER_START)
...
def get_pitcher_recent_stats(player_id, num_starts=5):
    try:
        ...
        k9_recent = min(k9_recent, MAX_K9)
...
def get_blended_pitcher_stats(player_id):
    ...
    if not current:
        cw, lw = 0.0, 1.0
    k9_current = min(current.get("k9", 8.0) if current else 8.0, MAX_K9)
    k9_last = min(last.get("k9", 8.0) if last else 8.0, MAX_K9)
    ...
    ip_current = min(current.get("avg_innings_per_start", 5.5) if current else 5.5, MAX_IP_PER_START)
    ip_last = min(last.get("avg_innings_per_start", 5.5) if last else 5.5, MAX_IP_PER_START)
    ip_blended = round((ip_current * cw) + (ip_last * lw), 2)
    ip_blended = min(ip_blended, MAX_IP_PER_START)
    if ip_blended < 0.1:
        ip_blended = 5.5
    if recent and current_starts >= 3:
        k9_final = min(round((k9_blended * 0.80) + (recent.get("k9_recent", k9_blended) * 0.20), 3), MAX_K9)
        era_final = round((era_blended * 0.80) + (recent.get("era_recent", era_blended) * 0.20), 3)
        ip_final = min(round((ip_blended * 0.80) + (recent.get("avg_ip_recent", ip_blended) * 0.20), 2), MAX_IP_PER_START)
    else:
        k9_final = k9_blended
...
def get_full_pitcher_profile(player_name, opponent_team, home_team):
    ...
    avg_ip = min(blended.get("avg_innings_per_start", 5.5), MAX_IP_PER_START)
    if avg_ip < 0.1:
        avg_ip = 5.5
    profile = {
        ...
        "avg_ip_season": avg_ip,
        "k9_recent": blended.get("k9", 8.0),
        "avg_k_recent": round(blended.get("k9", 8.0) / 9 * avg_ip, 2),
    ...
    logger.info("Profile: K9=" + str(profile["k9_season"]) + " ERA=" + str(profile["era"]) + " IP=" + str(avg_ip) + " oppAdj=" + str(profile["opp_k_adjustment"]) + " platoonAdj=" + str(platoon_adj))
    return profile
