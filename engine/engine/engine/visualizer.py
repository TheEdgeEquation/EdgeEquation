...
CBC_COLORS = {
    ...
    "grid":        (50, 35, 20),
    "accent":      (255, 165, 0),
    "text_primary":(255, 240, 210),
    "text_muted":  (180, 140, 90),
    "grade_ap":    (255, 215, 0),
    "grade_a":     (255, 165, 0),
    "grade_am":    (200, 130, 60),
    "border":      (80, 50, 25),
...
def _draw_equation_watermark(draw, w, h, colors):
    font = _load_font(13)
    snippets = [
        "P(X>k) = Σ e^-λ·λ^x/x!", "λ = E[X]", "EV = p·b - (1-p)",
        "σ² = λ", "n=10,000", "P(Over) = sim/n",
        "Kelly = (bp-q)/b", "edge = p_sim - p_imp",
    ]
    positions = [
        (30, 60), (200, 120), (500, 80), (800, 50), (1050, 110),
        (100, 500), (400, 560), (750, 520), (1000, 490),
    ]
...
def _draw_play_card(draw, img, play, x, y, w, colors, style):
    h_card = 130
    pad = 14
    ...
    badge_w = 56
    draw.rounded_rectangle([x + pad, y + pad, x + pad + badge_w, y + pad + 32], radius=5, fill=gc)
    draw.text((x + pad + badge_w // 2, y + pad + 6), grade, fill=(10, 10, 10), font=_load_font(15, bold=True), anchor="mt")

    score_x = x + pad + badge_w + 10
    ...
    prop_text = f"{play['display_line']} {play['prop_label']}  {play['display_odds']}  ·  {play['sport_label']}"
    draw.text((x + pad, y + pad + 62), prop_text, fill=colors["text_muted"], font=_load_font(14))
...
def _draw_header(draw, w, colors, style, date_str):
    if style == "cbc":
        title = "CASH BEFORE COFFEE"
        subtitle = "Powered by The Edge Equation Algorithm"
    else:
        title = "THE EDGE EQUATION"
        subtitle = f"{ALGO_VERSION}  ·  {date_str}"
...
def _draw_footer(draw, w, h, colors, style, plays):
    ...
    stats = [
        f"Run: {now.strftime('%I:%M %p CDT')}",
        f"Plays Graded: {len(plays)}",
        "Correlation Models: Active",
        "EV Sims: 10,000",
    ]
    draw.text((w // 2, h - 46), "  ·  ".join(stats), fill=colors["text_muted"], font=_load_font(12), anchor="mt")
...
def _draw_tier_header(draw, x, y, w, grade, label, colors):
    gc = _grade_color(grade, colors)
    draw.rectangle([x, y, x + w, y + 26], fill=colors["score_bg"])
    draw.line([(x, y), (x, y + 26)], fill=gc, width=3)
    draw.text((x + 14, y + 5), f"  {grade} TIER  –  {label.upper()}", fill=gc, font=_load_font(14, bold=True))
    return 32
...
def generate_main_graphic(plays: list[dict], style: str = "ee") -> str | None:
    ...
    _draw_equation_watermark(draw, GRAPHIC_WIDTH, GRAPHIC_HEIGHT, colors)
    ...
    for play in plays:
        ...
        if grade != last_grade:
            header_h = _draw_tier_header(draw, col_x, col_y, col_w, grade, play["play_label"], colors)
            if current_col == 0:
                y1 += header_h
            else:
                y2 += header_h
        ...
        consumed = _draw_play_card(draw, img, play, col_x, col_y, col_w, colors, style)
        if current_col == 0:
            y1 += consumed
        else:
            y2 += consumed
...
def generate_announce_graphic(games_today: list[dict], style: str = "ee") -> str:
    ...
    draw.text((w // 2, 62), date_str, fill=colors["text_muted"], font=_load_font(14), anchor="mt")
    draw.line([(40, 86), (w - 40, 86)], fill=colors["border"], width=1)

    y = 100
    for game in games_today[:8]:
        text = f"{game.get('sport_label',''):<6}  {game.get('away','')} @ {game.get('home','')}  ·  {game.get('time','')}"
        draw.text((w // 2, y), text, fill=colors["text_primary"], font=_load_font(15), anchor="mt")
        y += 30
    ...
    fname = f"{style}_announce_{datetime.now().strftime('%Y%m%d')}.png"
    fpath = os.path.join(OUTPUT_DIR, fname)
    img.save(fpath, "PNG")
    return fpath
...
def generate_results_graphic(results: list[dict], style: str = "ee") -> str:
    ...
    label = "RESULTS" if style == "ee" else "OVERNIGHT RESULTS"
    draw.text((w // 2, 24), label, fill=colors["accent"], font=_load_font(28, bold=True), anchor="mt")
    draw.text((w // 2, 62), date_str, fill=colors["text_muted"], font=_load_font(14), anchor="mt")
    draw.line([(40, 86), (w - 40, 86)], fill=colors["border"], width=1)
    ...
    for r in results:
        hit = r.get("hit", False)
        marker = "✓" if hit else "✗"
        marker_color = (80, 200, 120) if hit else (200, 80, 80)
        draw.text((w // 2 - 20, y), marker, fill=marker_color, font=_load_font(16, bold=True), anchor="rt")
    ...
    draw.text((w // 2, h - 26), tagline, fill=colors["accent"], font=_load_font(13, bold=True), anchor="mt")

    fname = f"{style}_results_{datetime.now().strftime('%Y%m%d')}.png"
    fpath = os.path.join(OUTPUT_DIR, fname)
    img.save(fpath, "PNG")
    return fpath
...
def generate_weekly_graphic(stats: dict) -> str:
    ...
    stat_items = [
        ...
        ("RECORD", f"{stats.get('hits', 0)}-{stats.get('misses', 0)}"),
        ("UNITS", f"+{stats.get('units', 0):.1f}u"),
        ("TOTAL PLAYS", str(stats.get('total', 0))),
    ]
    ...
    if stats.get("best_play"):
        draw.text((w // 2, y), f"Best Play: {stats['best_play']}", fill=colors["grade_ap"], font=_load_font(14), anchor="mt")
    if stats.get("worst_play"):
        draw.text((w // 2, y + 24), f"Toughest Beat: {stats['worst_play']}", fill=colors["text_muted"], font=_load_font(14), anchor="mt")

    draw.rectangle([0, h - 40, w, h], fill=colors["bg2"])
    draw.text((w // 2, h - 26), EE_TAGLINE, fill=colors["accent"], font=_load_font(13, bold=True), anchor="mt")

    fname = f"ee_weekly_{datetime.now().strftime('%Y%m%d')}.png"
    fpath = os.path.join(OUTPUT_DIR, fname)
    img.save(fpath, "PNG")
    return fpath
...
def generate_cbc_tease_graphic() -> str:
    ...
    fname = f"cbc_tease_{datetime.now().strftime('%Y%m%d')}.png"
    fpath = os.path.join(OUTPUT_DIR, fname)
    img.save(fpath, "PNG")
    return fpath
