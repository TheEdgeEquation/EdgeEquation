"""
engine/data_tracker.py
Persists daily plays and results to data/ as JSON.
Handles hit/miss tracking and weekly aggregation.
"""
import os
import json
import logging
...
def save_results(results: list[dict], style: str = "ee") -> str:
    ...
    logger.info(f"Results saved: {path} ({hits}/{len(results)})")
    return path
