...
def calculate_true_lambda_mlb(prop):
    try:
        ...
        logger.info("True lambda=" + str(round(true_lambda,3)))
            ...
            for game in date.get("games", []):
                try:
                    ...
                    if nrfi_grade:
                        ...
                        nrfi_plays.append({"player": away_pitcher + " / " + home_pitcher, "team": home_team, "opponent": away_team, "sport": "baseball_mlb", "sport_label": "MLB", "prop_label": "NRFI", "icon": "⚾", "line": 0.5, "over_odds": -135, "under_odds": 115, "implied_prob": NRFI_IMPLIED, "sim_prob": nrfi_prob, "edge": nrfi_edge, "grade": grade, "confidence_score": score, "play_label": label, "display_line": "NRFI", "display_odds": "-135", "true_lambda": nrfi_prob, "commence_time": commence, "umpire": umpire_name, "weather": weather.get("condition", ""), "dome": is_dome(home_team)})
                        logger.info("NRFI: " + away_team + " @ " + home_team + " " + grade + " edge=" + str(nrfi_edge))
                    if yrfi_grade:
                        grade, score, label = yrfi_grade
                        nrfi_plays.append({"player": away_pitcher + " / " + home_pitcher, "team": home_team, "opponent": away_team, "sport": "baseball_mlb", "sport_label": "MLB", "prop_label": "YRFI", "icon": "⚾", "line": 0.5, "over_odds": 115, "under_odds": -135, "implied_prob": YRFI_IMPLIED, "sim_prob": yrfi_prob, "edge": yrfi_edge, "grade": grade, "confidence_score": score, "play_label": label, "display_line": "YRFI", "display_odds": "+115", "true_lambda": yrfi_prob, "commence_time": commence, "umpire": umpire_name, "weather": weather.get("condition", ""), "dome": is_dome(home_team)})
